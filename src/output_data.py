"""
output_data.py — writers for tidy CSVs, Excel report, area text, and metadata bundle.

This module centralizes all file outputs for a single run. It produces a clean bundle:

  output/
    <run_name>/
      curves.csv              # tidy: V, y, curve (exp_act, fit_act, exp_inact, fit_inact)
...
- tidy_curves: pd.DataFrame with columns ["V", "y", "curve"] and rows for exp/fit variants
...
def _safe_name(name: str) -> str:
    # Very conservative cleaning for file-system safety
...
def _tidy_to_wide_with_six_cols(tidy_curves: pd.DataFrame) -> pd.DataFrame:
    """
    Convert tidy ["V","y","curve"] into wide 6 columns:
    V, y, exp_act, exp_inact, fit_act, fit_inact

    Rows are stacked (one per original tidy row). The specific curve column
    holds the value; other curve columns are NaN. 'y' duplicates that value.
    """
    required = {"V", "y", "curve"}
    ...
    for label in ("exp_act", "exp_inact", "fit_act", "fit_inact"):
        df = tidy_curves[tidy_curves["curve"] == label][["V", "y"]].copy()
        df["exp_act"] = np.nan
        df["exp_inact"] = np.nan
        df["fit_act"] = np.nan
        df["fit_inact"] = np.nan
    ...
    if len(frames):
        ...
        # Fallback: empty frame with correct columns
        out = pd.DataFrame(columns=cols)
    # Optional: sort by curve blocks then V (kept implicit by stacking order).
    return out
...
def write_outputs(
    *,
    ...
    run_name = _safe_name(config.run_name) if config.run_name else f"run-{_now_timestamp()}"
    ...
    if curves_csv is not None:
        _write_curves_csv(tidy_curves, curves_csv)
    ...
    if report_xlsx is not None:
        _write_report_xlsx(report_xlsx, tidy_curves, biomarkers, window_metrics)
...
def _write_curves_csv(tidy_curves: pd.DataFrame, path: Path) -> None:
    wide = _tidy_to_wide_with_six_cols(tidy_curves)
    wide.to_csv(path, index=False)


def _write_biomarkers_csv(biomarkers: pd.DataFrame, path: Path) -> None:
    # No strict schema; write whatever columns provided, but ensure indexless
    biomarkers.to_csv(path, index=False)


def _write_area_txt(path: Path, window_metrics: object) -> None:
    # Support both dataclass-like and dict-like access
    ...
    lines = [
        ...
        "Definition: ∫ min(m∞(V), h∞(V)) dV on [V_L_cap, V_R_cap], filled down to y=0.",
        "",
        f"Area: {area}",
...
def _write_report_xlsx(path: Path, tidy_curves: pd.DataFrame, biomarkers: pd.DataFrame, window_metrics: object) -> None:
    # Prepare a one-row dataframe for area metrics
    ...
    with pd.ExcelWriter(path, engine="openpyxl") as xw:
        _tidy_to_wide_with_six_cols(tidy_curves).to_excel(xw, index=False, sheet_name="curves")
...
if __name__ == "__main__":
    # Minimal smoke test writing to a temp folder
    import tempfile
    import numpy as np

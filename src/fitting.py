...
This module provides an implementation to:
    - Fit activation (m_inf) and inactivation (h_inf) curves with a Boltzmann (sigmoid) model.
    - Evaluate fitted curves on dense voltage grids for plotting/export.
    - Find 0/1/2 intersections between the two fitted curves.
    - Compute the physiologically meaningful "window area": integral[max(0, m_inf(V) - h_inf(V)] dV.
    - Extract biomarkers: V_{1/2}, slope factor k, plateaus (A_lo, A_hi), R^2, RMSE, and window peak.
...
Notes:
    ...
    - If non-linear least squares fails, an optional spline fallback can provide a smooth 
        curve for downstream metrics (flagged via result.method="spline_fit").
...
class CurveFitResult:
    ...
    success: bool
    method: str  # "curve_fit" or "spline_fit"
    r2: Optional[float]
    rmse: Optional[float]
...
class WindowMetrics:
    ...
    vmax_window: float  # max(m_inf − h_inf) within bounds
...
def _fit_generic(V: np.ndarray, y: np.ndarray, invert: bool, cfg: FittingConfig) -> CurveFitResult:
    ...
    try:
        ...
        if not cfg.enable_spline_fit:
            raise
        # Spline fit if curve_fit fails
        try:
            ...
            try:
                ...
                rmse = float(np.sqrt(ss_res / max(len(y), 1)))
                params = SigmoidParams(A_lo=float(popt[0]), A_hi=float(popt[1]), V_half=float(popt[2]), k=float(popt[3]), invert=invert)
                return CurveFitResult(params=params, success=True, method="spline_fit", r2=r2, rmse=rmse, y_pred_on_data=yhat, covariance=pcov)
            except Exception:
                # Return a spline-evaluated pseudo-constant sigmoid near data center
                ...
                r2 = 1.0 - ss_res / ss_tot if ss_tot > 0 else np.nan
                rmse = float(np.sqrt(ss_res / max(len(y), 1)))
                return CurveFitResult(params=params, success=False, method="spline_fit", r2=r2, rmse=rmse, y_pred_on_data=yhat, covariance=None)
        except Exception:
            # Return very basic params
...
def find_intersections(f_act: Callable[[np.ndarray], np.ndarray], f_inact: Callable[[np.ndarray], np.ndarray], V_grid: np.ndarray) -> List[float]:
    """Find all distinct roots of f(V) = m_inf(V) - h_inf(V) on the given grid using brentq bracketed by sign changes."""
    m = f_act(V_grid)
    h = f_inact(V_grid)
    diff = m - h
    roots: List[float] = []
...
def compute_window_area(
    ...
    """Compute Integral[max(0, m_inf - h_inf)] dV over [VL, VR]. If bounds are None, uses min/max of an adaptive grid.
...
def compute_biomarkers(
    ...
    *,
    ...
    # Find VL_cap: solve m_inf(V) = y_cap to the LEFT of V*
    Fa = f_act(Vg) - y_cap
    sFa = np.sign(Fa)
    ...
    # Find VR_cap: solve h_inf(V) = y_cap to the RIGHT of V*
    Fi = f_in(Vg) - y_cap
    sFi = np.sign(Fi)
    ...
    if Vcap.size >= 2:
        area = float(np.trapezoid(lower, Vcap))
    else:
